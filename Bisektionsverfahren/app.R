...
# 14. Apr. 2019  (nm)  Hintergrund, Literatur und Quellen angegeben. (0.5)
#
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("eps", "eps=", 5 * 10 ^ -4, min = 1 * 10 ^ -10, max =
                     0.1),
    ...
    mainPanel(tabsetPanel(
      ...
      tabPanel(
        ...
        fluidPage(
          ...
          p(
            "Die Idee des Bisektionsverfahren ist es, die Nullstelle einer stetigen Funktion \\(f: \\mathbf{R} \\to \\mathbf{R}\\) in einem Intervall \\([a; b]\\) einzuschliessen. Die Startwerte \\(a\\) und \\(b\\) sind dabei so zu wählen, dass zwischen ihnen (mindestens) eine Nullstelle liegt und sich daher die Vorzeichen von \\(f(a)\\) und \\(f(b)\\) unterscheiden."
          ),
          p("Liegt ein solches Intervall vor, so ist \\(f(a) \\cdot f(b) < 0\\). "),
          p(
            "Das Bisektionsverfahren teilt nun das Intervall \\([a; b]\\) in der Mitte auf. Dazu wird die als \\(c =\\frac{a+b}{2}\\) berechnet."
          ),
          p(
            "Nun sollte sich die Nullstelle entweder im Intervall \\([a; c]\\) oder im Intervall \\([c; b]\\) befinden. Dazu prüft man ob \\(f(a) \\cdot f(c) < 0\\) gilt, die Nullstelle somit im ersten Intervall liegt."
          ),
          
          p(
            "In diesem Falle ersetzt man \\(b\\) durch \\(c\\), andernfalls \\(a\\) durch \\(c\\) und beginnt von vorne."
          ),
          br(),
          h4("Abbruchkriterium"),
          p(
          ...
          p(
            "Ein weiteres Abbruckkriterium ist das Überprüfen einer maximalen Iterationsanzahl sinnvoll."
          ),
          br(),
          h4("Literatur/Quellen"),
          p(
            "- ",
            a("Wikipedia Artikel zum Thema Bisektion", href = "https://de.wikipedia.org/wiki/Bisektion")
          ),
          p(
...
server <- function(input, output, session) {
  ...
  output$functionPlot <- renderPlot({
    ...
    gf_line(
      ...
      gf_vline(xintercept = c(a, b),
               color = c(acolor, bcolor)) %>%
