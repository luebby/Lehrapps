...
tabPanel("Hintergrund", 
         fluidPage(
           ...
           fluidRow(column(12, "Wird ein Produkt innerhalb der zulässigen Toleranzen produziert, wird es akzeptiert, andernfalls muss es nachgearbeitet oder als Ausschuss entsorgt werden.")),
           fluidRow(column(12, "Wie viele Produkte außerhalb der Toleranzen liegen, hängt von der Standardabweichung ab.")),
           ...
           fluidRow(column(12, "Die Solllänge eines Stahlrohres sei 100 mm, der Toleranzbereich gehe von 99.95 mm bis 100.05 mm.")),
  ...
  output$Plotpnorm <- renderPlot({
    ...
    gf_dist("norm", mean = 100, sd = 0.05/sigma,
      ...
      gf_labs(title = bquote(Prozess~funktioniert~zum~Niveau~.(sigma)~sigma), 
              subtitle = paste("Erwarterer Ausschuss bei 1.000.000 Teilen: ", ausschuss),
              caption = "ohne Berücksichtigung einer evtl. Mittelwertsverschiebung") %>%
